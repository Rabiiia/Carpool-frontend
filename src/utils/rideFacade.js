...
import {CREATE_RIDE_ENDPOINT, DELETE_RIDE_ENDPOINT, GET_RIDE_ENDPOINT, RIDE_ENDPOINT, UPDATE_RIDE_ENDPOINT} from "../settings.js";
...
export const getRidesByDestination = async (destination) => {
  const response = await fetch(`${RIDE_ENDPOINT}?d=${destination}`);
  try {
    return await (await handleHttpErrors(response)).json();
  } catch (error) {
    console.log((await error).message);
...
export const getRide = async (id) => {
  ...
  const response = await fetch(`${RIDE_ENDPOINT}/${id}`, options);
  try {
    return await (await handleHttpErrors(response)).json();
  } catch (error) {
    console.log((await error).message);
